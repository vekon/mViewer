...
class NewCollectionComponent extends React.Component {
  ...
  getCappedData() {
    const partialUrl = this.props.currentDb + '/collection/' + this.state.name + '/isCapped?connectionId=' + this.props.connectionId;
    const getCappedDataCall = service('GET', partialUrl, '');
    ...
    if (calledFrom === 'getCappedData') {
      if(this.state._isMounted === true) {
        if(typeof(data.response) != 'undefined' && typeof(data.response.result) != 'undefined') {
          ...
          this.setState({size : typeof(data.response.result.size) != 'undefined' ? data.response.result.size : ''});
          this.setState({max : typeof(data.response.result.maxDocs) != 'undefined' && data.response.result.maxDocs !== '-1' ? data.response.result.maxDocs : ''});
        }
      }
    }
  }
