...
/**
 * Listens at a disconnect Request made by the user and destroys user id from the the mappings in
 * LoginController class and also from the session. The corresponding mongo instance is also
 * destroyed when all the tokenId corresponding to its user mapping are destroyed.
 *
    ...
    String response =
        new ResponseTemplate().execute(logger, connectionId, request, new ResponseCallback() {
          public Object execute() throws Exception {
            ...
            Set<String> existingConnectionIdsInSession =
                (Set<String>) session.getAttribute("existingConnectionIdsInSession");
