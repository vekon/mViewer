...
public class QueryExecutor {
	...
	public static JSONObject executeQuery(MongoDatabase db, MongoCollection<Document> mongoCollection,
			String collectionName, String command, String queryStr, String fields, String sortByStr, int limit,
			int skip, boolean allKeys) throws JSONException, ApplicationException {
		StringTokenizer strtok = new StringTokenizer(fields, ",");
		Document keysObj = new Document("_id", 1);
		while (strtok.hasMoreElements()) {
			keysObj.put(strtok.nextToken(), 1);
		}
		Document sortObj = (Document) JSON.parse(sortByStr);
	...
	private static JSONObject executeAggregate(MongoCollection mongoCollection, String queryStr)
			throws DatabaseException, JSONException {
		Document queryObj = (Document) JSON.parseAsArr(queryStr);
	...
	private static JSONObject executeCount(MongoCollection mongoCollection, String queryStr) throws JSONException {
		Document queryObj = (Document) JSON.parse(queryStr);
		long count = mongoCollection.count(queryObj);
		return ApplicationUtils.constructResponse(false, new Document("count", count));
	}

	private static JSONObject executeDistinct(MongoCollection<Document> mongoCollection, String queryStr)
		...
		if (queryObj.get("1") == null) {
			...
			iterator = mongoCollection.distinct((String) queryObj.get("0"), (Document) queryObj.get("1"), String.class)
					.iterator();
		}
	...
	private static JSONObject executeDrop(MongoCollection mongoCollection) throws JSONException {
	...
	private static JSONObject executeDropIndex(MongoCollection mongoCollection, String queryStr)
			throws JSONException, DatabaseException {
		Document indexInfo = (Document) JSON.parse(queryStr);
		if (indexInfo == null) {
	...
	private static JSONObject executeDropIndexes(MongoCollection MongoCollection) throws JSONException {
		MongoCollection.dropIndexes();
		return executeGetIndexes(MongoCollection);
	}

	private static JSONObject executeEnsureIndex(MongoCollection mongoCollection, String queryStr)
			throws JSONException, DatabaseException {
		Document queryObj = (Document) JSON.parseAsArr(queryStr);
	...
	private static JSONObject executeFindOne(MongoCollection mongoCollection, String queryStr) throws JSONException {
		Document queryObj = (Document) JSON.parseAsArr(queryStr);
	...
	private static JSONObject executeFind(MongoCollection mongoCollection, String queryStr, Document keysObj,
			Document sortObj, int limit, int skip, boolean allKeys) throws JSONException {
		Document queryObj = (Document) JSON.parse(queryStr);
	...
	private static JSONObject executeFindAndModify(MongoCollection mongoCollection, String queryStr, Document keysObj)
			throws JSONException {
		Document queryObj = (Document) JSON.parse(queryStr);
	...
	private static JSONObject executeGetIndexes(MongoCollection<Document> mongoCollection) throws JSONException {
	...
	private static JSONObject executeInsert(MongoCollection<Document> mongoCollection, String queryStr)
			throws JSONException {
		Document queryObj = (Document) JSON.parse(queryStr);
	...
	private static JSONObject executeGroup(MongoCollection<Document> mongoCollection, String queryString)
			throws JSONException {
		Document queryObj = (Document) JSON.parse(queryString);
	...
	private static JSONObject executeMapReduce(MongoCollection mongoCollection, String queryString)
			throws JSONException, InvalidMongoCommandException {
		Document queryObj = (Document) JSON.parseAsArr(queryString);
		...
		Document mapReduceOutput = (Document) mongoCollection.mapReduce(map, reduce).filter(query).sort(sort)
				.finalizeFunction(finalize).scope(scope).verbose(verbose);
	...
	private static JSONObject executeRemove(MongoCollection mongoCollection, String queryStr) throws JSONException {
		Document queryObj = (Document) JSON.parse(queryStr);
		DeleteResult deleteResult = mongoCollection.deleteMany(queryObj);
	...
	private static JSONObject executeUpdate(MongoCollection mongoCollection, String queryStr)
			throws JSONException, InvalidMongoCommandException {
		String reconstructedUpdateQuery = "{updateQueryParams:[" + queryStr + "]}";
		Document queryObj = (Document) JSON.parse(reconstructedUpdateQuery);
	...
	private static JSONObject executeStats(MongoDatabase db, String collectionName) throws JSONException {

		Document stats = db.runCommand(new Document("collStats", collectionName));
		return ApplicationUtils.constructResponse(false, stats);
	}

	private static JSONObject executeStorageSize(MongoDatabase db, String collectionName) throws JSONException {
		Integer storageSize = (Integer) db.runCommand(new Document("collStats", collectionName)).get("storageSize");
		return ApplicationUtils.constructResponse(false, new Document("storageSize", storageSize));
	}

	private static JSONObject executeTotalIndexSize(MongoDatabase db, String collectionName) throws JSONException {
		Integer totalIndexSize = (Integer) db.runCommand(new Document("collStats", collectionName))
				.get("totalIndexSize");
		return ApplicationUtils.constructResponse(false, new Document("totalIndexSize", totalIndexSize));
	}
